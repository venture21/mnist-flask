...
def create_compatible_model():
    """호환 가능한 MNIST MLP 모델 생성"""
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(784,)),  # 28*28 = 784 flattened input
...
try:
    ...
    try:
        ...
        # MLP를 위해 데이터를 flatten
        x_train = x_train.astype('float32') / 255.0
        x_train = x_train.reshape(-1, 784)  # Flatten to 784 dimensions
        ...
        print("호환 모델을 저장했습니다: mnist_mlp_model.keras")
...
def preprocess_image(image):
    ...
    # MLP를 위해 flatten (batch_size, 784)
...
def predict():
    ...
    try:
        ...
        # MLP용으로 flatten된 데이터를 다시 28x28로 reshape
        processed_display = processed_image_array[0].reshape(28, 28) * 255
...
def load_mnist():
    ...
    try:
        ...
        # 모델 예측을 위한 전처리 (MLP용으로 flatten)
        image_array = selected_image.astype('float32') / 255.0
        image_array = image_array.reshape(1, -1)  # Flatten to (1, 784)
